...
export default function Home() {
  ...
  const { isOpen: isMenuOpen, toggleMenu, closeMenu } = useMenu();
  const [listType, setListType] = useState<ListType>('google-style');

  useEffect(() => {
    loadListType();
    
    // Listen for list type changes
    const subscription = kernel.on('control-room:list-type-changed', (event: any) => {
      if (event.data?.listType) {
        setListType(event.data.listType);
        listLayoutManager.setListType(event.data.listType);
      }
    });
  ...
  const loadListType = async () => {
    try {
      const type = await listLayoutManager.getListType();
      setListType(type);
    } catch (error) {
    ...
    const init = async () => {
      try {
        ...
        try {
          ...
          voiceGlobalService.updateScreen('home');
    ...
    <View style={[styles.container, { backgroundColor: safeColors.background || '#000408' }]}>
      ...
      <View style={styles.header}>
        <Pressable onPress={toggleMenu} style={styles.menuButton}>
          <Icon name="menu" size={28} color={safeColors.primary} />
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  menuButton: { padding: 8, marginRight: 10 },
  logoText: { fontSize: 22, fontWeight: '900', letterSpacing: 4, flex: 1, textAlign: 'center' },
