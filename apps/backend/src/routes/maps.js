 ...
 * GET /api/maps/search?query=XXX (for compatibility)
 */
...
router.get('/search', async (req, res) => {
  try {
    const { query, location, provider = 'apple', radius = 5000, type } = req.query;
    ...
    const results = await Maps.searchLocation(query, locationObj, provider, parseInt(radius), type);
